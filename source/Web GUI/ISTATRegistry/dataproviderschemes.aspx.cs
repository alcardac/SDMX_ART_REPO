...
{
    public partial class dataproviderschemes : ISTATRegistry.Classes.ISTATWebPage
    {
        ...
        private ISdmxObjects GetSdmxObjects()
        {
            ...
            try
            {
                sdmxInput = wsModel.GetDataProviderScheme(new ArtefactIdentity(SearchBar1.ucID, SearchBar1.ucAgency, SearchBar1.ucVersion), true,true);

                if (SearchBar1.ucName.Trim() != string.Empty)
                {
                    foreach (IDataProviderScheme dataProviderScheme in sdmxInput.DataProviderSchemes)
                    {
                        if (localizedUtils.GetNameableName(dataProviderScheme).ToUpper().Contains(SearchBar1.ucName.Trim().ToUpper()))
                            mutableObj.AddDataProviderScheme(dataProviderScheme.MutableInstance);
                    }
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("DataProviderSchemeItemDetails.aspx?ACTION=INSERT");
        }
        ...
        protected void OnRowCommand(object sender, GridViewCommandEventArgs e)
        {
            switch (e.CommandName)
            {
                case "Details":
                    ...
                    Response.Redirect("DataProviderSchemeItemDetails.aspx?ACTION=UPDATE&" + stringIdentity);
                    break;
                case "xxxx":
                    break;
            }
